...
class Sensor:
    ...
    def in_fov(self, x):
        ...
        # a function that returns True if x lies in the sensor's field of view, 
        # otherwise False.
        ...
        # invalid value for angle as cannot divide by 0
        if pos_sensor[0] == 0:
            return False
    ...
    def get_hx(self, x):    
        ...
        if self.name == 'lidar':
            ...
            if pos_cam[0]==0:
                raise NameError("The x component cannot be zero.")
            else:
                hx = np.zeros((self.dim_meas, 1))
                hx[0,0] = self.c_i - (self.f_i*pos_cam[1]/pos_cam[0])
